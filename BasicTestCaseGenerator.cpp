...
int main() {
  ...
  uniform_int_distribution<> Nrand(1, 50000),
      Krand(1, 7000000);  /// Limites de nuestros random
  fstream buffer;
  ...
  for (int caso_actual = 0; caso_actual < numero_de_casos; caso_actual++) {
    ...
    int n = Nrand(
        gen);  /// Esto asigna a n un número aleatorio entre el rango 1-50000
    ...
    buffer << endl;  /// Importante siempre terminar nuestros archivos con un
                     /// salto de linea
    buffer.close();
